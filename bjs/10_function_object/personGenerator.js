const personGenerator = {
    ...
    randomPatronymic: function(gender) {// генерация отчества на основе пола
        ...
        switch(Patronymic){
            case 'Дмитрий':
            case 'Андрей':
                return Patronymic.split('й')[0] + ((gender==='Female') ? 'евна' : 'евич');// если слова имеют й - разделим их функцией split (изучил функцию самостоятельно) по букве й и возьмем первую часть слова. Затем просто использовуем суффикс "евна" если перед нами женский пол или "евич" - если мужской
            break;
            case 'Михаил':
                return (gender==='Female') ? 'Михайловна' : 'Михайлович';// согласно Русскому языку просиходит замена и на й. Это единственное такое слово, поэтому просто заменим его на правильную форму
            break;
            case 'Никита':
                return (gender==='Female') ? 'Никитична' : 'Никитич';// согласно Русскому языку, окончания будут ична и ич. Учитывая то, что слово такое одно - проще написать его полную форму, как исключение из правил
            break;
            default:

                return Patronymic + ((gender==='Female') ? 'овна' : 'ович');// если жен. пол - использовать суффикс "на", в противном случае перед нами муж.пол - используем "ич"
            }
            // по такому же принципу можно сделать Артёмовнич/Артёмовна, но в Русском языке часто считают е и ё одной буквой (+ в задании Артем без ё)
    ...
    randomBirthYear: function() {// функция для года (выбрал диапазон генерации с 1960 по 2002 год)

        return this.randomIntNumber(1960,2002);
